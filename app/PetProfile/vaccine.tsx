...
const VaccineScreen = () => {
  ...
  const [expiryDate, setExpiryDate] = useState(petData.rabiesExpiry);
  const [noRabies, setNoRabies] = useState(petData.rabiesExpiry === 'None');

  const isFormComplete = expiryDate.trim() !== '' || noRabies;

  const handleNext = () => {
    updatePetData({
      rabiesExpiry: noRabies ? 'None' : expiryDate,
    });
    router.push('/PetProfile/confirmation');
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.vaccineSection}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              setNoRabies(!noRabies);
              if (!noRabies) setExpiryDate('');
            }}
          >
